    ...
    <div className={isDashboard ? "w-full h-full flex flex-col min-h-0 overflow-hidden bg-background text-foreground" : "h-screen w-full flex flex-col bg-background text-foreground overflow-hidden font-sans"}>
      ...
      <main className="flex-1 flex flex-col md:flex-row min-h-0 pt-0 overflow-hidden bg-background">
        ...
        <section className="w-full md:w-[400px] lg:w-[450px] bg-card border-r border-slate-200 dark:border-border/30 flex flex-col h-full z-10 overflow-hidden flex-shrink-0 text-foreground">
          ...
          <div className="p-5 space-y-4 border-b border-slate-100 dark:border-border/20 bg-slate-50/20 dark:bg-slate-800/10">
            <div className="flex items-center justify-between">
              <h1 className="text-xl font-black text-primary flex items-center gap-1.5">
                {!isDashboard && (
                  <button
                    type="button"
            ...
            <div className="relative flex items-center bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-border/30 focus-within:border-primary focus-within:bg-white dark:focus-within:bg-slate-800 rounded-xl px-3 py-2.5 transition-all shadow-sm">
              <Search className="w-4 h-4 text-muted-foreground shrink-0 mr-2" />
              <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Tìm theo tên, địa chỉ, vị trí..."
                className="flex-1 bg-transparent text-sm outline-none text-foreground placeholder:text-muted-foreground/60 font-medium"
              />
              {search && (
                <button 
                  onClick={() => setSearch("")} 
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <select 
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-border/30 focus:border-primary focus:ring-1 focus:ring-primary rounded-xl px-3 py-2.5 text-xs font-semibold outline-none cursor-pointer text-foreground transition-colors"
                >
                  {CITIES.map(c => (
        ...
        <section className="flex-1 relative bg-background overflow-hidden min-h-[300px] md:min-h-0">
          ...
          {selected && (
            <div className="absolute bottom-8 left-8 w-80 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-border/30 p-5 shadow-2xl z-20 animate-in fade-in slide-in-from-bottom-4 duration-300">
